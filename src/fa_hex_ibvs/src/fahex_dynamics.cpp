...
void LinearForcesCallback(const geometry_msgs::Vector3::ConstPtr& fcs)
{
	linear_forces(0) = fcs->x;
	linear_forces(1) = fcs->y;
	linear_forces(2) = fcs->z;
}
...
{
	ros::init(argc, argv, "fahex_dynamics");
	ros::NodeHandle nh;
	ros::Rate loop_rate(100);
	
	ros::Subscriber thrust_sub = nh.subscribe("quad_linear_forces",100,&LinearForcesCallback);
	...
	while(ros::ok())
	{
		...
		//Linear dynamics
		force = (RotationMatrix(attitude_position).inverse() * (quad_mass * gravity * e3)) - linear_forces + dist; 
