...
int main(int argc, char *argv[])
{
    ...
    //ros::Subscriber sub = nh.subscribe("raspicam_node/image/Compressed", 100, imageCallback); //Real camera

	//ros::Subscriber attitude_sub = nh.subscribe("quad_attitude",100, &attitude_callback);
	ros::Subscriber attitude_sub = nh.subscribe("attitude_estimates",100, &attEstCallback);
