...
void posCallback(const geometry_msgs::Vector3::ConstPtr& pos)
{
	position(0) = pos->x;
	position(1) = pos->y;
	position(2) = pos->z;
...
{
    ros::init(argc, argv, "td_linear");
	ros::NodeHandle nh;
	ros::Rate loop_rate(100);	

    //ROS publishers and subscribers
    ros::Subscriber quad_position_sub = nh.subscribe("quad_position",100, &posCallback);
    ros::Subscriber quad_attitude_sub = nh.subscribe("quad_attitude",100, &attCallback);


    ros::Publisher position_est_pub = nh.advertise<geometry_msgs::Twist>("position_estimates",100);
    ros::Publisher position_est_err_pub = nh.advertise<geometry_msgs::Vector3>("position_td_error",100);
    ros::Publisher linVel_BF_est_pub = nh.advertise<geometry_msgs::Vector3>("lin_vel_BF_estimates",100);


    geometry_msgs::Twist position_est_var; //linear: x, y, z    angular: vel_x, vel_y vel_z
    geometry_msgs::Vector3 linVel_BF_est_var;
