...
//void raspicamCallback(const sensor_msgs::CompressedImageConstPtr& msg)
//{
...
void flagCallback(const std_msgs::Bool::ConstPtr& fg)
{
	flag = fg->data;
}
...
int main(int argc, char *argv[])
{
    ...
    //ros::Subscriber img_sub = nh.subscribe("/raspicam_node/image/compressed", 1, &raspicamCallback);
	ros::Subscriber attitude_sub = nh.subscribe("quad_attitude",100, &attitude_callback);
	ros::Subscriber flag_sub = nh.subscribe("flag_ibvs",100,&flagCallback);
